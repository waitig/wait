...
<head>
    ...
    <title><?php wp_title(waitig_gopt('waitig_delimiter'), true, 'right');
        echo get_option('blogname');
        if (is_home()) echo waitig_gopt('waitig_delimiter'), get_option('blogdescription');
...
<header id="header" class="header">
    <div id="nav-header" class="navbar">
        <ul class="nav">
            ...
            <div class="nav-right">
                <?php if (waitig_gopt('waitig_nav_link')) { ?>
                    <div class="nav-right-link"><?php echo waitig_gopt('waitig_nav_link'); ?></div><?php } ?>
                <div class="search-expand" style="display: none;">
                    <div class="search-expand-inner">
                        <form method="get" class="searchform themeform"
                              onsubmit="location.href='<?php echo home_url('/search/'); ?>' + encodeURIComponent(this.s.value).replace(/%20/g, '+'); return false;"
                              action="/">
        ...
        <div class="toptip"><strong class="text-success" style="float:left"><i class="fa fa-volume-up"></i> </strong>
            <div id="news" style="float:left">
                <ul id="topNotice" onmouseout="gstart()" onmouseover="gstop()"
                    style="margin-left:15px"><?php echo waitig_gopt('waitig_tui'); ?></ul>
            </div>
        </div>
    </div>
