...
if( !waitig_gopt('d_tougao_b') ) die('off');

global $wpdb;
$last_post = $wpdb->get_var("SELECT post_date FROM $wpdb->posts WHERE post_type = 'post' ORDER BY post_date DESC LIMIT 1");

if ( current_time('timestamp') - strtotime($last_post) < (waitig_gopt('d_tougao_time')?waitig_gopt('d_tougao_time'):240) ){
    die('sofast');
}
...
if ($status != 0) { 
    if( waitig_gopt('d_tougao_mailto') ) wp_mail(waitig_gopt('d_tougao_mailto'), "站长，有新投稿！ ".$title, $post_content);
    die('success');
}else{
    die('fail');
}
